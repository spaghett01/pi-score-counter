...
function updateAllStats(callback) {
    ...
    getAllStatsData(function() {
        ...
        for (var i = 0; i < _lstPlayers.length; i++) {
            //debugger;
            if (_lstPlayers[i].name.indexOf("GUEST") > -1) {
...
function setBannerText() {
    ...
    if (_lstPlayersMostWonGames[0].games_won > 0) {
        ...
        currentStatTemplate = currentStatTemplate.replace(/STATS/g, "Top Wins");
    ...
    if (_lstPlayersWinLostRatio[0].win_lost_ratio > 0) {
        currentStatTemplate = statTemplate
        currentStatTemplate = currentStatTemplate.replace(/STATS/g, "W/L Ratio");
        currentStatTemplate = currentStatTemplate.replace(/NAME1/g, _lstPlayersMostWonGames[0].name);
    ...
    if (_lstPlayersShutoutWins[0].shutout_wins > 0) {
        currentStatTemplate = statTemplate
        currentStatTemplate = currentStatTemplate.replace(/STATS/g, "Most Shutout Wins");
        currentStatTemplate = currentStatTemplate.replace(/NAME1/g, _lstPlayersMostWonGames[0].name);
    ...
    if (_lstPlayersMostWonGames[0].overtime_wins > 0) {
        currentStatTemplate = statTemplate
        currentStatTemplate = currentStatTemplate.replace(/STATS/g, "Most Overtime Wins");
        currentStatTemplate = currentStatTemplate.replace(/NAME1/g, _lstPlayersMostWonGames[0].name);
    ...
    if (_lstPlayersPlayedGames[0].games_played > 0) {
        currentStatTemplate = statTemplate
        currentStatTemplate = currentStatTemplate.replace(/STATS/g, "Most Games Played");
    ...
    if (_longestGameInfo.maxScores > 0) {
        ...
        currentStatTemplate = currentStatTemplate.replace(/STATS/g, "Longest Game");
        currentStatTemplate = currentStatTemplate.replace(/CONTENT/g, _longestGameInfo.maxScores + " points - " + _longestGameInfo.namePlayer1 + " (" + _longestGameInfo.scorePlayer1 + ") against " + _longestGameInfo.namePlayer2 + " (" + _longestGameInfo.scorePlayer2 + ") " + _longestGameInfo.daysDiff + (_longestGameInfo.daysDiff > 1 ? " days ago" : " day ago"));
    ...
    if (_lastShutoutInfo.date != 0) {
        currentStatTemplate = uniqueStatTemplate;
        currentStatTemplate = currentStatTemplate.replace(/STATS/g, "Last Shutout");
        currentStatTemplate = currentStatTemplate.replace(/CONTENT/g, _lastShutoutInfo.namePlayer1 + " (" + _lastShutoutInfo.scorePlayer1 + ") against " + _lastShutoutInfo.namePlayer2 + " (" + _lastShutoutInfo.scorePlayer2 + ") " + _lastShutoutInfo.daysDiff + (_lastShutoutInfo.daysDiff > 1 ? " days ago" : " day ago"));
...
function setStatsScreen() {
    ...
    if (isInSeason()) {
        $("#nav-stats-title").html('Statistics of ranked players - <span class="yellow-text">' + Season.Name + '</span>');
    } else {
        $("#nav-stats-title").html('Statistics of ranked players - <span class="yellow-text">Of all time</span>');
    }
    ...
    if (lstPlayersStats.length == 0) {
        $("#table-stats-section").html('<h1 class="orange-text center-align" id="no-player-ranked-message">No ranked player</h1>' + '<h3 class="orange-text center-align">' + getMinimumGamesForRanking() + ' games are required to be ranked</h3>');
        return;
    }
    ...
    statsTemplate += '<th>Player</th>'; // Name
    statsTemplate += '<th>ELO</th>'; // ELO
    statsTemplate += '<th>GP</th>'; // Games played
    statsTemplate += '<th>W</th>'; // Win
    statsTemplate += '<th>L</th>'; // Lost
    statsTemplate += '<th>W/L</th>'; // W/L Ratio
    statsTemplate += '<th>W-OT</th>'; // Overtime win
    statsTemplate += '<th>L-OT</th>'; // Overtime lost
    statsTemplate += '<th>W-SO</th>'; // Shutout win
    statsTemplate += '<th>L-SO</th>'; // Shutout lost
